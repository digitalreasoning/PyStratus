...
from cassandra.service import CASSANDRA_NODE
...
SUBSTITUTIONS = ["%SEEDS%", "%INITIAL_TOKEN%"]
...
firewall to the master node. (May be specified multiple times.)"),
  make_option("--security-group", metavar="SECURITY_GROUP", action="append",
    default=None, help="Additional security groups within which the instances \
...
def parse_options_and_config(command, argv, option_list=[], extra_arguments=(),
  ...
  cluster = get_cluster(cloud_provider)(cluster_name, 
                    config_dir, opt.get("region", DEFAULT_REGION))
...
def execute(command=None, argv=[]):
  ...
  if command == 'details':
    ...
    service.list()
      ...
      (opt, args, service) = parse_options_and_config(command, argv, BASIC_OPTIONS)
      service.configure_nodes(opt.get('ssh_options'))

  elif command == 'print-ring':
      (opt, args, service) = parse_options_and_config(command, argv, BASIC_OPTIONS)
      service.print_ring(opt.get('ssh_options'))
    ...
    instance_templates = []
    if len(args) == 2:
      number_of_nodes = int(args[1])
      instance_template = InstanceTemplate((CASSANDRA_NODE,), number_of_nodes,
