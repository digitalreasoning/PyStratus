...
import ConfigParser
from hadoop import VERSION
from cloud.cluster import get_cluster
from cloud.service import get_service
from cloud.service import InstanceTemplate
from hadoop_cassandra_hybrid.service import NAMENODE
from hadoop_cassandra_hybrid.service import SECONDARY_NAMENODE
from hadoop_cassandra_hybrid.service import JOBTRACKER
from hadoop_cassandra_hybrid.service import DATANODE
from hadoop_cassandra_hybrid.service import TASKTRACKER
from hadoop_cassandra_hybrid.service import CASSANDRA_NODE
from hadoop_cassandra_hybrid.service import HADOOP_CASSANDRA_NODE
...
def print_usage(script):
  print """Usage: %(script)s CLUSTER COMMAND [OPTIONS]
where COMMAND and [OPTIONS] may be one of:
  ...
  launch-cluster (NUM_SLAVES|         launch a master and NUM_SLAVES slaves or
  ...
  migrate-snapshots                   migrate snapshots from CLUSTER to
                                         a new availability zone
  delete-snapshots                    delete snapshots for CLUSTER
  login                               log in to the master in CLUSTER over SSH
  proxy                               start a SOCKS proxy on localhost into the
                                        CLUSTER
  push FILE                           scp FILE to the master in CLUSTER
  exec CMD                            execute CMD on the master in CLUSTER
  terminate-cluster                   terminate all instances in CLUSTER
  delete-cluster                      delete the group information for CLUSTER
  delete-storage                      delete all storage volumes for CLUSTER
  update-slaves-file                  update the slaves file on the CLUSTER
                                        master
...
def parse_options_and_config(command, argv, option_list=[], extra_arguments=(),
  ...
  (options_dict, args) = parse_options(command, argv, option_list, expected_arguments,
                                       unbounded_args)
...
def parse_options(command, argv, option_list=[], expected_arguments=(),
                  unbounded_args=False):
  """
...
configuration file.""" % (" ".join([command] + expected_arguments[1:]),
                          config_file_name)
...
def execute(command=None, argv=[]):
  ...
  if command == 'details':
    ...
    (opt, args, service) = parse_options_and_config(command, argv, LAUNCH_OPTIONS)
    ...
    if len(args) == 2:
      number_of_slaves = int(args[1])
      instance_templates = [
        InstanceTemplate((NAMENODE, SECONDARY_NAMENODE, JOBTRACKER, HADOOP_CASSANDRA_NODE), 1,
                         get_image_id(service.cluster, opt),
                         opt.get('instance_type'), opt.get('key_name'),
                         opt.get('public_key'), opt.get('user_data_file'),
                         opt.get('availability_zone'), opt.get('user_packages'),
      ...
      for i in range(len(args) / 2):
        ...
        # hybrid cluster - data nodes are also cassandra nodes
        if DATANODE in roles:
          roles.append(CASSANDRA_NODE)
    ...
    service.launch_cluster(instance_templates, config_dir,
      ...
      (opt, args, service) = parse_options_and_config(command, argv, BASIC_OPTIONS)
      service.start_cassandra(opt.get('ssh_options'))

  elif command == 'stop-cassandra':
      (opt, args, service) = parse_options_and_config(command, argv, BASIC_OPTIONS)
      service.stop_cassandra(opt.get('ssh_options'))

  elif command == 'login':
    (opt, args, service) = parse_options_and_config(command, argv, SSH_OPTIONS)
    ...
    service.terminate_cluster(service, opt["force"])
  ...
  elif command == 'snapshot-storage':
    (opt, args, service) = parse_options_and_config(command, argv, FORCE_OPTIONS)
    service.snapshot_storage(args[0], force=opt["force"])

  elif command == 'list-snapshots':
    (opt, args, service) = parse_options_and_config(command, argv, FORCE_OPTIONS)
    service.list_snapshots(args[0])

  elif command == 'migrate-snapshots':
    (opt, args, service) = parse_options_and_config(command, argv, FORCE_OPTIONS,
                                                    ("ZONE",))
    service.migrate_snapshots(args[0], args[1], get_config_dir(opt), force=opt["force"])

  elif command == 'delete-snapshots':
    (opt, args, service) = parse_options_and_config(command, argv, FORCE_OPTIONS)
    service.delete_snapshots(args[0], force=opt["force"])
